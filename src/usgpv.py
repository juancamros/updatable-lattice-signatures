...
class usgpv:
    ...
    @staticmethod
    def LLL_token_reduction(self, token: np.ndarray, delta=0.99):
        """
        Aplica LLL a las últimas w filas del token.

        Si
            Delta = [Delta_1]
                    [Delta_2]

        con Delta_1 de dimensión mbar x m y Delta_2 de dimensión w x m,
        LLL calcula una matriz unimodular P tal que

            Delta_2_red = P @ Delta_2.
        ...
        Esto permite absorber U^{-1} en la nueva clave pública y mantiene
        la parte R de la trapdoor original sin modificar.

        Output:
            token_red : token reducido T = U @ Delta
            P         : transformación unimodular w x w
            U         : transformación unimodular completa m x m
        """
        ...
        if token.shape != (self.m_param, self.m_param):
            raise ValueError(
                f'El token debe tener dimensión '
                f'{self.m_param} x {self.m_param}, '
                f'pero tiene shape {token.shape}'
            )

        # Nos aseguramos de no mandar floats a LLL
        if not np.all(np.equal(token, np.round(token))):
            raise ValueError('LLL necesita una matriz con entradas enteras')

        token = np.asarray(np.round(token), dtype=object)

        # Separamos el token por filas:
        #
        # Delta = [ Delta_1 ]   mbar filas
        #         [ Delta_2 ]   w filas
        #
        Delta_1 = token[:self.mbar_param, :]
        Delta_2 = token[self.mbar_param:, :]

        # SymPy trabaja exactamente con enteros.
        # Cada FILA se interpreta como un vector de la base.
        Delta_2_sym = Matrix(
            [[int(x) for x in row] for row in Delta_2]
        )

        # Convertimos delta a racional exacto.
        delta_sym = Rational(str(delta))
        ...
        try:
            ...
            # Delta_2_red = P * Delta_2
            #
            Delta_2_red_sym, P_sym = Delta_2_sym.lll_transform(
                delta=delta_sym
            )

        except DMRankError:
            raise ValueError(
                'Las últimas w filas del token no son linealmente '
                'independientes, por lo que no forman una base válida '
                'para aplicar LLL de esta forma.'
            )

        # Comprobación exacta
        if P_sym * Delta_2_sym != Delta_2_red_sym:
            raise ValueError('Error interno: P @ Delta_2 != Delta_2_red')

        # Pasamos de SymPy a NumPy con dtype object para no tener
        # overflow de int64 si los coeficientes crecen.
        P = np.array(P_sym.tolist(), dtype=object)

        Delta_2_red = np.array(
            Delta_2_red_sym.tolist(),
            dtype=object
        )

        # Reconstruimos el token reducido
        token_red = np.vstack([
            Delta_1,
            Delta_2_red
        ])

        # Construimos U = diag(I_mbar, P)
        U = np.zeros(
            (self.m_param, self.m_param),
            dtype=object
        )

        U[:self.mbar_param, :self.mbar_param] = np.eye(
            self.mbar_param,
            dtype=object
        )

        U[self.mbar_param:, self.mbar_param:] = P

        # Comprobación final exacta:
        # token_red = U @ token
        if not np.array_equal(
            ...
            raise ValueError(
                'La reducción no cumple token_red = U @ token'
            )

        return token_red, P, U
